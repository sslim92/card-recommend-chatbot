...
추가 대형 모델 도입에 따른 GPU 메모리 부담을 고려해 기존 임베딩 모델을 활용하는 방향을 선택했습니다. 카드 검색에 사용하는 E5 임베딩 모델을 업종 분류에서도 공유하고, 질문·업종 벡터를 결합하는 별도의 분류 신경망을 학습했습니다. 이를 통해 업종 분류를 위한 별도 언어모델 로딩을 피했습니다. 메모리 절감량이나 응답 속도를 별도로 측정하지는 않았습니다.
